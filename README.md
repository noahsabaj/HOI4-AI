...
Preprocessing is one implementation shared by training and deployment: `views` resizes with an exact area average, in float32, on the GPU when one is free. Changing that resampler invalidates prepared datasets and any checkpoint trained on them.
