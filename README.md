...
Other projects on this PC run their own jobs on the second PC (and other computers) with the fleet project's `fleet` command, which replaced this project's `peer` on 2026-09-25. It reaches the second PC through this worker, and relies on three things here: the `job` operation with kind `project` on observer connections, `Run-Job.ps1`'s `jobs/<id>.json` and `jobs/<id>.log`, and the evaluation reservation files (`--eval-dir`: `queue/`, `granted/`, `done/`), through which a job needing the GPU has the recorder yield between games. Keep those compatible, or tell the user before changing them; `tests/test_fleet_contract.py` holds them. Recording, game control and the live view stay this worker's own.
...
`train-memory` reads ahead: eight threads read each batch's cells from the cache into pinned memory while the GPU trains on the batch before. Each update's steps through the memory replay from CUDA graphs, and evaluation keeps its results on the GPU until the end. The results are the same as before, to the bit. `features.train_memories` trains several cells on the same batches at once, each on its own CUDA stream. Keep the cache on a fast disk: a run reads the training games' cells nine times (18.6 GB a pass for the 44-game cache), and D: on this PC is a USB drive that reads 0.44 GB/s.
...
`--preset` picks a named arena instead of the plain one, and `preview-map` draws it from its files. There are seven, all 12x8 provinces a side in 8 states, each an exact half-turn mirror, painted with stock terrain, relief, rivers on province borders, trees, cities holding the victory points, a trunk railway, and borders that wander: `plains` (open farmland), `river` (a large river behind each border), `passes` (a mountain range on the border crossed by two valleys), `marsh` (a marsh and lake in the middle, forests on the wings), `bay` (the sea cuts in at the border, leaving an isthmus) `salient` (the border bends round a bulge on each side) and `ford` (a large river along the whole border, but for one ford). `--seed` draws another map of the same design. STATUS.md says what each changes.
