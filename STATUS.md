...
| Red's AI taking ground | Same run, debug tooltip at 101 in-game days | **None.** `Surrender level: 0.00` with controlled and owned victory points equal. Blue sat still by design; Red's AI took no Blue province in three months of game time, at four times march speed, with a supply hub in every state |
...
- Work out why Red's AI takes no ground. On the rescaled map it held position for three months of game time with nothing opposing it but a stationary Blue. Province size is no longer the explanation, so the next suspects are the ones the audit left standing: no military commander of any kind, no doctrine and no army experience to buy one with, and stock `common/ai_strategy` files that reference vanilla states and log `invalid state id` for almost every entry they carry. A `front_control` strategy with `execute_order = yes` is the documented override if none of those is enough.
